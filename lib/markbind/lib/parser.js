...
const BOILERPLATE_DIRECTORY_NAME = '_markbind/boilerplates';
...
function calculateBoilerplateFilepath(filePath, pathWithinBoilerplate, config) {
  const fileBase = calculateNewBaseUrl(filePath, config.rootPath, config.baseUrlMap).relative;
  return path.resolve(fileBase, BOILERPLATE_DIRECTORY_NAME,
                      pathWithinBoilerplate || path.basename(filePath));
}
...
Parser.prototype._preprocess = function (node, context, config) {
  ...
  if (element.name === 'include') {
    ...
    if (isBoilerplate) {
      actualFilePath = calculateBoilerplateFilepath(filePath, element.attribs.boilerplate, config);
    }
    ...
    element.attribs.src = filePath;
    let actualFilePath = filePath;
    if (_.hasIn(element.attribs, 'boilerplate')) {
      actualFilePath = calculateBoilerplateFilepath(filePath, element.attribs.boilerplate, config);
    }
...
Parser.prototype._parse = function (node, context, config) {
  ...
  case 'dynamic-panel': {
    ...
    const fileExists = utils.fileExists(element.attribs.src)
                    || utils.fileExists(calculateBoilerplateFilepath(element.attribs.src,
                                                                     element.attribs.boilerplate, config));
    if (fileExists) {
...
Parser.prototype.includeFile = function (file, config) {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (!utils.fileExists(file)) { // if a boilerplate file is an addressable page in site.json
      // search for file with the same name in the boilerplate folder
